...
export class DataService {
  ...
  // --- LOGGING ---
  logAction(userId: string, module: string, action: 'AGREGAR' | 'EDITAR' | 'ELIMINAR', detail: string) {
  ...
  createSale(sale: Sale): void {
    ...
    // LOGGING: If it's a pending payment, log it
    if (sale.paymentMethod === PaymentMethod.PENDING) {
        this.logAction(
            sale.userId, 
            'Punto de Venta', 
            'AGREGAR', 
            `Pendiente de Pago generado a: ${sale.customerName || 'Cliente'} - Monto: S/ ${sale.totalAmount.toFixed(2)}`
        );
    }
  }
  ...
  markSaleAsPaid(saleId: string, paymentMethod: PaymentMethod = PaymentMethod.CASH, collectedByUserId?: string, collectedByShiftId?: string): void {
    ...
    if (sale) {
      ...
      // LOGGING: Log who collected the debt
