...
const POS: React.FC<POSProps> = ({ products, currentUser, currentShift, onSaleComplete }) => {
  ...
  const confirmCheckout = () => {
    ...
    if (paymentMethod === PaymentMethod.COLLABORATOR_ADVANCE && !customerName.trim()) {
    ...
    } else if (paymentMethod === PaymentMethod.COLLABORATOR_ADVANCE) {
        detailMsg = `Método: Adelanto (Colaboradores)\nColaborador: ${customerName}`;
    }
    ...
    <div className="flex flex-col lg:flex-row lg:h-[calc(100vh-100px)] h-auto gap-4">
      ...
      <div className="w-full lg:w-96 flex flex-col bg-white dark:bg-neutral-800 rounded-xl shadow-lg border border-neutral-200 dark:border-neutral-700 lg:h-full h-auto">
        ...
        <div className="p-4 bg-neutral-50 dark:bg-neutral-900 border-t border-neutral-200 dark:border-neutral-700 space-y-4">
            <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                    {Object.values(PaymentMethod)
                        .filter(m => m !== PaymentMethod.MIXED && m !== PaymentMethod.COLLABORATOR_ADVANCE) // Render standard methods first
                        .map(method => (
                        <button
                            key={method}
                            onClick={() => {
                    ...
                    {/* Botón Adelanto (Colaboradores) */}
                    <button
                        onClick={() => {
                            setPaymentMethod(PaymentMethod.COLLABORATOR_ADVANCE);
                            setMixedCashAmount(''); // Reset mixed state
                        }}
                        className={`col-span-2 px-2 py-2 text-xs rounded border transition-colors flex items-center justify-center gap-2 ${
                            paymentMethod === PaymentMethod.COLLABORATOR_ADVANCE
                            ? 'bg-amber-600 border-amber-700 text-white font-bold'
                            : 'bg-white dark:bg-neutral-800 border-neutral-300 dark:border-neutral-600 text-amber-600 dark:text-amber-400 hover:bg-neutral-100'
                        }`}
                    >
                        Adelanto (Colaboradores)
                    </button>
            ...
            {paymentMethod === PaymentMethod.COLLABORATOR_ADVANCE && (
                <div>
                    <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Nombre del Colaborador</label>
                    <div className="flex items-center bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-600 rounded-lg px-3 py-2">
                        <UserIcon className="w-4 h-4 text-gray-400 mr-2" />
                        <input
                            type="text"
                            value={customerName}
                            onChange={(e) => setCustomerName(e.target.value)}
                            placeholder="Ej. Juan Pérez (Colaborador)"
                            className="bg-transparent w-full text-sm focus:outline-none"
                        />
                    </div>
                </div>
