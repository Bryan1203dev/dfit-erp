...
const Memberships: React.FC<MembershipsProps> = ({ currentUser, currentShift, onUpdate }) => {
  ...
  const [memberships, setMemberships] = useState<MembershipRecord[]>(() => {
      const all = dataService.getMemberships();
      return isAdmin ? all : all.filter(m => currentShift && m.shiftId === currentShift.id);
  });
  const [abonos, setAbonos] = useState<AbonoRecord[]>(() => {
      const all = dataService.getAbonos();
      return isAdmin ? all : all.filter(a => currentShift && a.shiftId === currentShift.id);
  });
  ...
  const [memMixedCashAmount, setMemMixedCashAmount] = useState('');
  const [abonoMixedCashAmount, setAbonoMixedCashAmount] = useState('');
  ...
  React.useEffect(() => {
      const allM = dataService.getMemberships();
      const allA = dataService.getAbonos();
      setMemberships(isAdmin ? allM : allM.filter(m => currentShift && m.shiftId === currentShift.id));
      setAbonos(isAdmin ? allA : allA.filter(a => currentShift && a.shiftId === currentShift.id));
  ...
  const handleSaveMembership = (e: React.FormEvent) => {
      ...
      if (memFormData.paymentMethod === PaymentMethod.MIXED) {
          const abonoVal = Number(memFormData.abono);
          const cash = parseFloat(memMixedCashAmount) || 0;
          const app = abonoVal - cash;
          if (cash < 0 || cash > abonoVal) {
              alert("El monto en efectivo para el pago excepcional no es válido.");
              return;
          }
  ...
  const handleSaveAbono = (e: React.FormEvent) => {
      ...
      let paymentDetails = undefined;
      if (abonoFormData.paymentMethod === PaymentMethod.MIXED) {
          const amountVal = Number(abonoFormData.amount);
          const cash = parseFloat(abonoMixedCashAmount) || 0;
          const app = amountVal - cash;
          if (cash < 0 || cash > amountVal) {
              alert("El monto en efectivo para el pago excepcional no es válido.");
              return;
          }
          paymentDetails = { cash, app };
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="number" required className="w-full p-2 rounded border dark:bg-neutral-900 dark:border-neutral-600 dark:text-white"
                                value={memFormData.price ?? ''} onChange={e => setMemFormData({...memFormData, price: e.target.value === '' ? undefined : parseFloat(e.target.value)})} />
                        </div>
                        <div>
                            ...
                            <input type="number" required className="w-full p-2 rounded border dark:bg-neutral-900 dark:border-neutral-600 dark:text-white font-bold text-gold-600"
                                value={memFormData.abono ?? ''} onChange={e => setMemFormData({...memFormData, abono: e.target.value === '' ? undefined : parseFloat(e.target.value)})} />
                        </div>
                        <div className="col-span-2">
                            ...
                            <select 
                                ...
                                onChange={(e) => {
                                    ...
                                    setMemFormData({...memFormData, paymentMethod: method});
                            ...
                            >
                                ...
                                <option value={PaymentMethod.MIXED}>Método Excepcional (Efec/App)</option>
                            </select>
                        </div>
                        {memFormData.paymentMethod === PaymentMethod.MIXED && (
                            <div className="col-span-2 bg-purple-50 dark:bg-purple-900/20 p-3 rounded-lg border border-purple-200 dark:border-purple-800 space-y-2">
                                ...
                                <div className="grid grid-cols-2 gap-2 items-center">
                                    <div>
                                        ...
                                        <input 
                                            type="number" 
                                            value={memMixedCashAmount}
                                            onChange={(e) => setMemMixedCashAmount(e.target.value)}
                                            placeholder="0.00"
                                            className="w-full p-2 text-sm rounded border border-purple-300 dark:border-purple-600 bg-white dark:bg-neutral-100 text-neutral-800 dark:text-neutral-800 focus:outline-none focus:ring-1 focus:ring-purple-500 text-right font-bold"
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <div className="w-full p-2 text-sm rounded border border-transparent bg-purple-100 dark:bg-purple-900/40 text-purple-800 dark:text-purple-200 text-right font-bold">
                                            {(((Number(memFormData.abono) || 0) - (parseFloat(memMixedCashAmount) || 0))).toFixed(2)}
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                         ...
                         <div>
                            <label className="block text-xs text-gray-500 mb-1">Abono (S/)</label>
                            <input type="number" required className="w-full p-2 rounded border dark:bg-neutral-900 dark:border-neutral-600 dark:text-white font-bold text-gold-600"
                                value={abonoFormData.amount ?? ''} onChange={e => setAbonoFormData({...abonoFormData, amount: e.target.value === '' ? undefined : parseFloat(e.target.value)})} />
                        </div>
                        <div>
                            <label className="block text-xs text-gray-500 mb-1">Método de Pago</label>
                            <select className="w-full p-2 rounded border dark:bg-neutral-900 dark:border-neutral-600 dark:text-white"
                                ...
                                onChange={(e) => {
                                    const method = e.target.value as PaymentMethod;
                                    setAbonoFormData({...abonoFormData, paymentMethod: method});
                                    if (method !== PaymentMethod.MIXED) {
                                        setAbonoMixedCashAmount('');
                                    }
                                }}
                        ...
                        {abonoFormData.paymentMethod === PaymentMethod.MIXED && (
                            <div className="col-span-2 bg-purple-50 dark:bg-purple-900/20 p-3 rounded-lg border border-purple-200 dark:border-purple-800 space-y-2 animate-in fade-in slide-in-from-top-2">
                                <p className="text-[10px] text-purple-800 dark:text-purple-300 font-bold uppercase mb-1">Distribución del Pago</p>
                                <div className="grid grid-cols-2 gap-2 items-center">
                                    <div>
                                        <label className="block text-[10px] text-gray-500 mb-1">Efectivo (S/)</label>
                                        <input 
                                            type="number" 
                                            value={abonoMixedCashAmount}
                                            onChange={(e) => setAbonoMixedCashAmount(e.target.value)}
                                            placeholder="0.00"
                                            className="w-full p-2 text-sm rounded border border-purple-300 dark:border-purple-600 bg-white dark:bg-neutral-100 text-neutral-800 dark:text-neutral-800 focus:outline-none focus:ring-1 focus:ring-purple-500 text-right font-bold"
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-[10px] text-gray-500 mb-1">Aplicativo (S/)</label>
                                        <div className="w-full p-2 text-sm rounded border border-transparent bg-purple-100 dark:bg-purple-900/40 text-purple-800 dark:text-purple-200 text-right font-bold">
                                            {(((Number(abonoFormData.amount) || 0) - (parseFloat(abonoMixedCashAmount) || 0))).toFixed(2)}
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <tr key={m.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-700/50">
                                    ...
                                    <td className="px-6 py-4 text-right space-x-2">
                                        <button 
                                            onClick={() => {
                                                setMemFormData(m);
                                                setMemMixedCashAmount(m.paymentDetails ? m.paymentDetails.cash.toString() : '');
                                                setIsMemFormOpen(true);
                                            }}
                                ...
                                <tr key={ab.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-700/50">
                                    ...
                                    <td className="px-6 py-4 text-right space-x-2">
                                        <button 
                                            onClick={() => {
                                                setAbonoFormData(ab);
                                                setAbonoMixedCashAmount(ab.paymentDetails ? ab.paymentDetails.cash.toString() : '');
                                                setIsAbonoFormOpen(true);
                                            }}
                                            className="text-blue-600 hover:text-blue-800 dark:text-blue-400"
                                            title="Editar"
