...
import { PlayCircle, StopCircle, DollarSign, Wallet, ClipboardCheck, LayoutList, Package, Users, CreditCard, ShieldCheck, PlusCircle, FileSpreadsheet, Filter } from 'lucide-react';
...
const ShiftManager: React.FC<ShiftManagerProps> = ({ currentUser, currentShift, onShiftChange }) => {
  ...
  const getFilterTimestamps = () => {
      if (!startDate) return { start: 0, end: Infinity };
      const start = new Date(startDate + 'T00:00:00').getTime();
      const end = endDate ? new Date(endDate + 'T23:59:59').getTime() : new Date(startDate + 'T23:59:59').getTime();
      return { start, end };
  };

  const { start: filterStart, end: filterEnd } = getFilterTimestamps();

  const relevantSales = useDateFilter
      ? dataService.getSales().filter(s => s.timestamp >= filterStart && s.timestamp <= filterEnd)
      : (isAdmin ? dataService.getTodaySales() : (currentShift ? dataService.getShiftSales(currentShift.id) : []));

  const relevantMemberships = useDateFilter
      ? dataService.getMemberships().filter(m => m.timestamp >= filterStart && m.timestamp <= filterEnd)
      : (isAdmin ? dataService.getTodayMemberships() : (currentShift ? dataService.getShiftMemberships(currentShift.id) : []));

  const relevantAbonos = useDateFilter
      ? dataService.getAbonos().filter(a => a.timestamp >= filterStart && a.timestamp <= filterEnd)
      : (isAdmin ? dataService.getTodayAbonos() : (currentShift ? dataService.getShiftAbonos(currentShift.id) : []));

  const relevantExpenses = useDateFilter
      ? dataService.getExpenses().filter(e => e.timestamp >= filterStart && e.timestamp <= filterEnd)
      : (isAdmin ? dataService.getTodayExpenses() : (currentShift ? dataService.getShiftExpenses(currentShift.id) : []));

  const relevantExtras = useDateFilter
      ? dataService.getExtras().filter(e => e.timestamp >= filterStart && e.timestamp <= filterEnd)
      : (isAdmin ? dataService.getTodayExtras() : (currentShift ? dataService.getShiftExtras(currentShift.id) : []));

  // Splits for Details (Excluding collaborator advances as they are only accounting records)
  const productSales = relevantSales.filter(s => !s.isFreePass && s.paymentMethod !== PaymentMethod.COLLABORATOR_ADVANCE);
  const freePassSales = relevantSales.filter(s => s.isFreePass);

  // --- Specific Cash Breakdown for Closing Report ---
  const cashMembershipsOnly = relevantMemberships.reduce((sum, m) => {
      if (m.paymentMethod === PaymentMethod.CASH) return sum + m.abono;
      if (m.paymentMethod === PaymentMethod.MIXED && m.paymentDetails) return sum + m.paymentDetails.cash;
      return sum;
  }, 0);
  const cashAbonos = relevantAbonos.reduce((sum, a) => {
      if (a.paymentMethod === PaymentMethod.CASH) return sum + a.amount;
      if (a.paymentMethod === PaymentMethod.MIXED && a.paymentDetails) return sum + a.paymentDetails.cash;
      return sum;
  }, 0);
  ...
  const currentStartCash = useDateFilter
      ? dataService.getShifts().filter(s => s.startTime >= filterStart && s.startTime <= filterEnd).reduce((sum, s) => sum + s.startCash, 0)
      : (currentShift ? currentShift.startCash : 0); 
  
  // Total Income (Cash)
  const totalCashIncome = totalCashMembershipIncome + cashProducts + cashFreePass + cashExtras + currentStartCash;
  
  // Separate cash and app expenses
  const totalExpensesCash = relevantExpenses
      .filter(e => e.paymentMethod === PaymentMethod.CASH)
      .reduce((acc, e) => acc + e.amount, 0);

  const totalExpensesApp = relevantExpenses
      .filter(e => e.paymentMethod === PaymentMethod.APP || e.paymentMethod === PaymentMethod.TRANSFER)
      .reduce((acc, e) => acc + e.amount, 0);

  const totalExpenses = relevantExpenses.reduce((acc, e) => acc + e.amount, 0);
  
  const totalNeto = totalCashIncome - totalExpensesCash;
  const deposit = totalNeto - currentStartCash;

  // --- Digital Income (App + Transfer + Mixed App Part) ---
  const digitalMemberships = relevantMemberships.reduce((sum, m) => {
      if (m.paymentMethod === PaymentMethod.APP || m.paymentMethod === PaymentMethod.TRANSFER) return sum + m.abono;
      if (m.paymentMethod === PaymentMethod.MIXED && m.paymentDetails) return sum + m.paymentDetails.app;
      return sum;
  }, 0);
  const digitalAbonos = relevantAbonos.reduce((sum, a) => {
      if (a.paymentMethod === PaymentMethod.APP || a.paymentMethod === PaymentMethod.TRANSFER) return sum + a.amount;
      if (a.paymentMethod === PaymentMethod.MIXED && a.paymentDetails) return sum + a.paymentDetails.app;
      return sum;
  }, 0);
  ...
  const totalDigitalBeforeExpenses = digitalMemberships + digitalAbonos + digitalSales + digitalExtras;
  const totalDigital = totalDigitalBeforeExpenses - totalExpensesApp;
  ...
  const memAppTotal = relevantMemberships.reduce((sum, m) => {
      if (m.paymentMethod === PaymentMethod.APP) return sum + m.abono;
      if (m.paymentMethod === PaymentMethod.MIXED && m.paymentDetails) return sum + m.paymentDetails.app;
      return sum;
  }, 0) + relevantAbonos.reduce((sum, a) => {
      if (a.paymentMethod === PaymentMethod.APP) return sum + a.amount;
      if (a.paymentMethod === PaymentMethod.MIXED && a.paymentDetails) return sum + a.paymentDetails.app;
      return sum;
  }, 0);
  ...
  const generateFinalReport = () => {
    ...
    const summaryData = [
        ...
        ["(=) TOTAL INGRESOS (Efectivo)", totalCashIncome],
        ["(-) TOTAL EGRESOS (Efectivo)", totalExpensesCash],
        ["(=) TOTAL NETO CAJA", totalNeto],
        ["", ""],
        ["EFECTIVO A DEPOSITAR", deposit],
        ["+ TOTAL INGRESOS APP (App + Transf + Extras)", totalDigitalBeforeExpenses],
        ["(-) TOTAL EGRESOS (Aplicativo)", totalExpensesApp],
        ["(=) TOTAL NETO INGRESOS APP", totalDigital],
        ["", ""],
        ["TOTAL GENERADO (Depósito + Neto Digital)", totalGeneratedToday]
    ];
    ...
    const memData = relevantMemberships.map(m => {
        let metodoStr: string = m.paymentMethod;
        if (m.paymentMethod === PaymentMethod.MIXED && m.paymentDetails) {
            metodoStr = `Mixto (E:${m.paymentDetails.cash}/A:${m.paymentDetails.app})`;
        }
        return {
    ...
    const abonoData = relevantAbonos.map(a => {
        let metodoStr: string = a.paymentMethod;
        if (a.paymentMethod === PaymentMethod.MIXED && a.paymentDetails) {
            metodoStr = `Mixto (E:${a.paymentDetails.cash}/A:${a.paymentDetails.app})`;
        }
        return {
    ...
    const expData = relevantExpenses.map(e => ({
        ...
        Metodo: e.paymentMethod || PaymentMethod.CASH,
        Monto: e.amount
    }));
    ...
    const histData = relevantSales.filter(s => s.isPaid).map(s => {
        ...
        let detalleStr = '';
        if (s.paymentMethod === PaymentMethod.PENDING || s.paymentMethod === PaymentMethod.COLLABORATOR_ADVANCE || s.customerName) {
            detalleStr = s.customerName || '';
        }
        return {
            Fecha: new Date(s.timestamp).toLocaleString(),
            Vendedor: getUserName(s.userId),
      ...
      <div className="bg-white dark:bg-neutral-800 rounded-2xl shadow-xl overflow-hidden border border-neutral-200 dark:border-neutral-700">
        ...
        <div className="p-6">
            ...
            <div className="mb-6 p-4 bg-neutral-50 dark:bg-neutral-800/50 rounded-xl border border-neutral-200 dark:border-neutral-700">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h4 className="text-sm font-bold text-gray-800 dark:text-white uppercase tracking-wider flex items-center gap-2">
                            ...
                            Filtro Avanzado de Reportes (Rango de Fechas)
                        </h4>
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            Genere reportes de fechas específicas o rangos de días anteriores.
                        </p>
                    </div>
                    <div className="flex items-center gap-2">
                        <label className="text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">Activar Filtro:</label>
                        <input 
                            type="checkbox" 
                ...
                {useDateFilter && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-4 pt-4 border-t border-neutral-200 dark:border-neutral-700 animate-in slide-in-from-top-2 duration-200">
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">Fecha Inicio / Un Solo Día</label>
                            <input 
                                type="date" 
                                value={startDate}
                                onChange={(e) => setStartDate(e.target.value)}
                                className="w-full p-2.5 text-sm rounded-lg border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-900 dark:text-white focus:ring-2 focus:ring-gold-500 focus:outline-none"
                            />
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">Fecha Fin (Opcional)</label>
                            <input 
                                type="date" 
                                value={endDate}
                        ...
                        <div className="flex items-end">
                            <button
                                type="button"
                                onClick={() => { setStartDate(''); setEndDate(''); setUseDateFilter(false); }}
                                className="w-full py-2.5 bg-neutral-200 hover:bg-neutral-300 dark:bg-neutral-700 dark:hover:bg-neutral-600 text-gray-800 dark:text-white rounded-lg text-xs font-bold uppercase transition-colors"
                            >
                                Limpiar Filtro
                            </button>
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <SummaryCard title={useDateFilter ? "Total Generado (Rango)" : "Total Generado Hoy"} amount={totalGeneratedToday} icon={DollarSign} color="text-gold-500" />
                <SummaryCard 
                    title={useDateFilter ? "FONDO INICIAL (SUMA)" : "FONDO INICIAL"} 
                    amount={currentStartCash} 
                    sub={useDateFilter ? "Total Aperturas en Rango" : "Monto de Apertura"}
                    icon={Wallet} 
                    color="text-green-500" 
                />
                <SummaryCard title={useDateFilter ? "Total Aplicativos (Rango)" : "Total Aplicativos"} amount={totalDigital} icon={DollarSign} color="text-purple-500" />
                <SummaryCard title={useDateFilter ? "Total Por Cobrar (Rango)" : "Total Por Cobrar"} amount={salesPending} icon={ClipboardCheck} color="text-red-500" />
            </div>
                    ...
                    <div className="animate-in fade-in duration-300">
                        ...
                        <div className="space-y-1 text-sm bg-white dark:bg-neutral-800 p-4 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-700">
                            ...
                            <div className="flex justify-between p-2 border-b border-dashed border-gray-200 dark:border-gray-700 bg-emerald-50/50 dark:bg-emerald-900/10">
                             ...
                             {/* EXPENSES */}
                             <div className="flex flex-col p-3 mt-2 bg-red-50/30 dark:bg-red-950/10 rounded-lg">
                                <div className="flex justify-between items-center">
                                    <span className="text-red-500 font-bold">(-) TOTAL EGRESOS (Efectivo)</span>
                                    <span className="font-bold text-red-500">S/ {totalExpensesCash.toFixed(2)}</span>
                                </div>
                                {totalExpensesApp > 0 && (
                                    <div className="flex justify-between items-center text-xs text-neutral-400 mt-1 pt-1 border-t border-neutral-200/50 dark:border-neutral-700/50">
                                        <span>(-) Egresos por Aplicativo (no restan de caja)</span>
                                        <span>S/ {totalExpensesApp.toFixed(2)}</span>
                                    </div>
                                )}
                             </div>
                             ...
                             {/* TOTAL NETO */}
                              <div className="flex justify-between p-3 bg-neutral-100 dark:bg-neutral-700/50 rounded-lg mt-2 border-l-4 border-gold-500">
                                 <span className="text-gray-800 dark:text-white font-bold uppercase">(=) TOTAL NETO CAJA</span>
                         ...
                         {/* DIGITAL SUMMARY */}
                         <div className="mt-6 p-4 bg-purple-50 dark:bg-purple-900/10 rounded-lg border border-purple-100 dark:border-purple-900/30">
                            <div className="flex justify-between items-center">
                                 <div className="flex items-center gap-2">
                                     <CreditCard className="text-purple-600 dark:text-purple-400" />
                                     <span className="text-purple-800 dark:text-purple-300 font-bold text-sm uppercase">TOTAL INGRESOS APP (Neto)</span>
                                 </div>
                                 <span className="text-2xl font-bold text-purple-700 dark:text-purple-400">S/ {totalDigital.toFixed(2)}</span>
                            </div>
                            {totalExpensesApp > 0 && (
                                <div className="mt-2 pt-2 border-t border-purple-200/50 flex justify-between items-center text-xs text-purple-600/80 dark:text-purple-400/80">
                                    <span>Ingresos App Brutos: S/ {totalDigitalBeforeExpenses.toFixed(2)} | Egresos App Restados: S/ {totalExpensesApp.toFixed(2)}</span>
                                </div>
                            )}
                         </div>
                    </div>
