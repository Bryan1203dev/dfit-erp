...
import { Edit2, Trash, Plus, Archive, CheckCircle, AlertOctagon, FileDown } from 'lucide-react';
...
const Inventory: React.FC<InventoryProps> = ({ products, currentUser, onUpdate }) => {
  ...
  const handleModalConfirm = () => {
    if (productToDelete) {
        ...
        // Fallback for other modal actions if any in future
        setModalOpen(false);
    }
  };
  ...
  const handleAuditSubmit = () => {
    ...
    // If no blocking discrepancies (diff == 0 for all checked), save audit (although logic implies 0 diff doesnt need action, but we save record)
    Object.entries(auditValues).forEach(([productId, value]) => {
        const physicalQty = Number(value);
        const product = products.find(p => p.id === productId);
        if (product) {
  ...
  return (
    ...
    <Modal 
        isOpen={modalOpen} 
        onClose={() => { setModalOpen(false); setProductToDelete(null); }} 
                ...
                <form onSubmit={handleSaveProduct} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select className="w-full p-2 rounded border dark:bg-neutral-900 dark:border-neutral-600 dark:text-white" value={currentProduct.category || ProductCategory.OTHER} onChange={e => setCurrentProduct({...currentProduct, category: e.target.value as ProductCategory})}>
                                {Object.values(ProductCategory).map(c => <option key={c} value={c}>{c}</option>)}
                            </select>
                        </div>
                         <div>
                            <label className="block text-xs text-gray-500 mb-1">Precio (S/)</label>
                            <input type="number" step="0.01" required className="w-full p-2 rounded border dark:bg-neutral-900 dark:border-neutral-600 dark:text-white" value={currentProduct.price || ''} onChange={e => setCurrentProduct({...currentProduct, price: parseFloat(e.target.value)})} />
                        </div>
                         <div>
                            <label className="block text-xs text-gray-500 mb-1">Stock Inicial</label>
                            <input type="number" required className="w-full p-2 rounded border dark:bg-neutral-900 dark:border-neutral-600 dark:text-white" value={currentProduct.stock} onChange={e => setCurrentProduct({...currentProduct, stock: parseInt(e.target.value)})} />
                        </div>
                         <div>
                            <label className="block text-xs text-gray-500 mb-1">Stock Mínimo</label>
                            <input type="number" required className="w-full p-2 rounded border dark:bg-neutral-900 dark:border-neutral-600 dark:text-white" value={currentProduct.minStock || ''} onChange={e => setCurrentProduct({...currentProduct, minStock: parseInt(e.target.value)})} />
                        </div>
                    </div>
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-2">
                                ...
                                <div className="text-center p-2 bg-neutral-200 dark:bg-neutral-800 rounded border border-gray-300 dark:border-gray-600">
                                    ...
                                    <input 
                                        ...
                                        onChange={(e) => setAuditValues({...auditValues, [p.id]: parseInt(e.target.value) || 0})}
                                    />
                                </div>
                            </div>
