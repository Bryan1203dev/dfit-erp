...
import { Sale, Product, ProductCategory, User, Shift, Role, PaymentMethod } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ sales, products, currentUser, currentShift }) => {
  ...
  // Update: "Ventas Hoy" only sums PAID product sales (excluding Free Pass, Pending, and Collaborator Advances)
  const totalRevenue = relevantSales
    .filter(s => !s.isFreePass && s.isPaid && s.paymentMethod !== PaymentMethod.COLLABORATOR_ADVANCE)
    .reduce((acc, curr) => acc + curr.totalAmount, 0);

  const totalTransactions = relevantSales.filter(s => s.paymentMethod !== PaymentMethod.COLLABORATOR_ADVANCE).length;
  ...
  relevantSales.forEach(sale => {
    if (!sale.isFreePass && sale.isPaid && sale.paymentMethod !== PaymentMethod.COLLABORATOR_ADVANCE) {
