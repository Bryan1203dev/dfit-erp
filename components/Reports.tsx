...
const Reports: React.FC<ReportsProps> = ({ sales, currentUser, onUpdate }) => {
  ...
  const filteredHistory = (isAdmin ? sales : sales.filter(s => currentShift && s.shiftId === currentShift.id)).filter(s => {
  ...
  const visibleHistorySalesForDropdown = isAdmin ? sales : sales.filter(s => currentShift && s.shiftId === currentShift.id);
  const uniqueUserIds = Array.from(new Set(visibleHistorySalesForDropdown.map(s => s.userId)));
                        ...
                        {tab === 'LOGS' ? (
                            logs.sort((a,b) => b.timestamp - a.timestamp).map(l => (
                                <tr key={l.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-700/50">
                                    ...
                                    <td className="px-6 py-4 text-xs font-mono text-gray-600 dark:text-gray-400">{l.products || '-'}</td>
