...
const App: React.FC = () => {
  ...
  const NavItem = ({ id, label, icon: Icon }: { id: ViewState, label: string, icon: any }) => (
    <button
      ...
      className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${view === id
          ...
          <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-white dark:bg-neutral-900 border-r border-neutral-200 dark:border-neutral-800 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-200 ease-in-out flex flex-col`}>
            <div className="p-6">
              <div className="flex items-center gap-2 mb-8">
                <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-transparent hover:border-red-600 transition-all duration-300 shadow-lg shadow-red-900/20">
                ...
                <div>
                  ...
                  <span className="text-xs text-red-600 uppercase font-bold tracking-widest">Gym ERP</span>
                </div>
              </div>
